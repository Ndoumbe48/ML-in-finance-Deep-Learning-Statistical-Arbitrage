...
print(f"Number of cryptocurrencies: {len(SYMBOLS_120)}")
print(f"After cleaning: {len(SYMBOLS_CLEAN)} cryptos\n")
...
def fetch_prices_binance_4h(symbol, retry_count=3):
    """
    Fetch 4-hour OHLCV data from Binance API for a given symbol.
    ...
    Returns:
        pd.DataFrame: DataFrame with close prices, or None if failed
    """
...
def load_all_prices(symbols):
    """
    Load 4-hour price data for all symbols from Binance.
    
    Args:
        symbols (list): List of cryptocurrency symbols
        
    Returns:
        pd.DataFrame: DataFrame with prices for each symbol (rows=timestamps, columns=symbols)
    """
    print("\n" + "="*80)
    ...
    if not all_prices:
        print("\n❌ No cryptocurrencies loaded!")
        return None
    
    prices_df = pd.concat(all_prices, axis=1)
    prices_df = prices_df.sort_index()
    
    print(f"\n✓ Loaded {len(prices_df.columns)} cryptos")
    print(f"  Candles: {len(prices_df)}")
    
    return prices_df

def fetch_circulating_supply(symbol, retry_count=3):
    """
    Fetch circulating supply for a cryptocurrency from CoinGecko API.
    ...
    Returns:
        float: Circulating supply, or np.nan if failed
    """
...
def load_all_supplies(symbols):
    """
    Load circulating supply for all symbols from CoinGecko.
    
    Args:
        symbols (list): List of cryptocurrency symbols
        
    Returns:
        dict: Dictionary mapping symbols to their circulating supply
    """
    print("\n" + "="*80)
    print("FETCHING SUPPLIES (CoinGecko)")
    print("="*80 + "\n")
...
def calculate_marketcap_and_filter_top40(prices_df, supplies_dict, n=40):
    """
    Calculate market cap at each timestamp and identify the top n cryptocurrencies.
    
    Args:
        prices_df (pd.DataFrame): DataFrame with prices (rows=timestamps, columns=symbols)
        supplies_dict (dict): Dictionary mapping symbols to circulating supply
        n (int): Number of top cryptocurrencies to select (default: 40)
        
    Returns:
        tuple: (top_n_df, top_n_history)
            - top_n_df: DataFrame with 1 for top n cryptos, NaN otherwise
            - top_n_history: List of dicts with date and top symbols at each timestamp
    """
    print("\n" + "="*80)
    print(f"FILTERING TOP {n}")
    print("="*80 + "\n")
...
def main():
    """
    Main pipeline: fetch prices, get supplies, calculate market caps, and identify top 40 cryptos.
    """
